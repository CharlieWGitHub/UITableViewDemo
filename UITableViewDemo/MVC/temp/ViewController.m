...
//预设高度
        if (model.dPictureAdr.count>0) {
//      中间高度
            NSInteger midH = ((model.dPictureAdr.count -1) / 3 + 1) * ((SCREEN_WIDTH-40)/3 + 10);
            NSLog(@"%ld",(long)midH);
...
//      底部高度
        if (model.comments.count>0) {
            NSInteger bottomH = 20 * model.comments.count + 20 + 42;
            NSLog(@"%ld",(long)bottomH);
       
        }else{
            NSInteger bottomH = 42;
            NSLog(@"%ld",(long)bottomH);
