...
@interface ViewController ()<UITableViewDelegate,UITableViewDataSource,cDynamicCellDelegate>
...
- (void)loadNewData{
    NSMutableArray *activityDynamics = [NSMutableArray arrayWithCapacity:0];
    for (int i=0; i<8; i++) {
        if (i&1) {
            [activityDynamics addObject:[self getAcDic]];
        }else{
            [activityDynamics addObject:[self getDyDic]];
        }
    }
...
- (void)loadMoreData{
   ...
   NSMutableArray * temp = [NSMutableArray arrayWithArray:[CellModel mj_objectArrayWithKeyValuesArray:activityDynamics]];
    
    for (CellModel * model in temp) {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([model.type isEqualToString:@"Activity"]) {
        ...
        if (model.getContextHeight>120) {
            if (model.isShow ==YES) {
                return model.getMidH + model.getBottomH + model.getContextHeight + 60 + 60;
            }else{
//            实际需要的高度
//          暂时展示的高度 120 文字高度 + 70 头像部分的高度
                return model.getMidH + model.getBottomH + 120 +70;
            }
        }else{
            return model.getMidH + model.getBottomH + model.getContextHeight + 70;
        }
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
        ...
        if ([model.type isEqualToString:@"Activity"]) {
            ...
            [cell setCellContentWithModel:model WithIndexPat:indexPath];
            cell.delegate  = self;
            return cell;
        }
}
...
/*
 * 找到对应的model 更改model的属性 刷新 tableview-cell
 */
- (void)shrinkageContextWithIndexPat:(NSIndexPath *)indexPath{
   
//    NSIndexPath *selIndexPath=[NSIndexPath indexPathForRow:indexPath.row inSection:0];
    CellModel * model = self.dataArray[indexPath.row];
    NSLog(@"----%d",model.isShow);
    model.isShow =! model.isShow;
    [self.myTab reloadData];
//    [self.myTab reloadRowsAtIndexPaths:[NSArray arrayWithObjects:selIndexPath,nil] withRowAnimation:UITableViewRowAnimationNone];
}
